...
def get_ngram(N=1,lang='EN'):
    ...
    source = './dictionaries/%s/%s' % (lang,ngrams[N])
    try:
        data = open(source).read().splitlines()
    except:
        print("Can't open dictionary file %s" % source)
        exit(2)
    else:
        dictio = {}
...
# def fitness(text,lang='EN'):
#     chi = 0
#     # for i in range(1,4):
#     freq_an = block_freq(text,1)
#     dictio = get_ngram(1,lang)
